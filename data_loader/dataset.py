...
class ReconDataset(Dataset):
    """
    Simple dataset class for reconstruction tasks.

    This dataset prepares data for reconstruction by converting input data to tensors:
    - Converts input data to tensors
    - Handles optional labels for conditional generation

    Args:
        ds (np.ndarray): Input data
    """
...
class ClassificationDataset(Dataset):
    ...
    This dataset prepares data for classification by converting input data to tensors:
    - Converts input data to tensors
    - Handles optional labels for conditional generation
    ...
    def __init__(self, ds: np.ndarray, labels: np.ndarray):
        self.ds = ds
        self.labels = labels
    ...
    def __getitem__(self, idx) -> tuple[torch.Tensor, torch.Tensor]:
        return torch.tensor(self.ds[idx], dtype=torch.float32), torch.tensor(
            self.labels[idx], dtype=torch.long
        )

class MyLatentAutoregressiveDataset(Dataset):
    """
    Dataset class for autoregressive modeling of latent space representations.

    This dataset prepares data for sequence prediction by shifting inputs and adding special tokens:
    - Shifts data right and adds start/end tokens ([start,2,3,4] predicts [2,3,4,end])
    - Handles optional masking of tokens for training
    - Supports conditional generation with optional labels

    Args:
        data (np.ndarray): Input sequence data
    ...
    def __init__(
        self, data: np.ndarray, y: np.ndarray | None = None, prob_missing: float = 0.0
    ):
        """
