...
from data_loader.dataset import MyLatentAutoregressiveDataset, MyLatentClassificationDataset
...
def get_latent_space_IDs(
    ...
    if latent_space_model.use_dvae_vq:
        # get idx from one hot encoded z_q
        min_encoding_indices = torch.argmax(z_q, dim=2)
...
def create_latent_space_dataset_VQ_VAE_IDs(
    ...
    len_ds = len(loader.dataset) - seq_len + 1
...
def create_autoreg_ds(
    ...
    data_module: Union[WeldingDataModule, CNCDataModule, ECGDataModule],
    task: Literal["reconstruction", "classification"] = "reconstruction",
    seq_len: int = 1,
    device: str = "cpu",
    prob_unk_token: float = 0.0,
    seq_prediction_task: bool = True
) -> tuple[MyLatentAutoregressiveDataset, MyLatentAutoregressiveDataset, MyLatentAutoregressiveDataset]:
    """
    ...
    data_module.setup(seq_len=seq_len)
    train_loader = data_module.train_dataloader()
    val_loader = data_module.val_dataloader()
    test_loader = data_module.test_dataloader()
