...
class CustomLightningMLFlowLogger(MLFlowLogger):
    ...
    def __init__(self, experiment_name = "default", run_name = None, tracking_uri = None, tags = None, save_dir = "./mlruns", log_model = False, prefix = "", artifact_location = None, run_id = None):
...
class MyMLFlowLogger:
    ...
    def __init__(self, experiment_name: str = "DEFAULT", run_name: str | None = None):
        ...
        """
        self.MLFLOW_SERVER_URL = os.getenv("MLFLOW_SERVER_URL", "http://localhost:5000")
        # mlflow credentials
        self._user = os.getenv("MLFLOW_USER", "admin")
        self._password= os.getenv("MLFLOW_PASSWORD", "admin")
        
        # s3 endpoint for artifacts
        self._s3_endpoint = os.getenv("MLFLOW_S3_ENDPOINT_URL", "http://localhost:9000")
        self._aws_access_key_id = os.getenv("MLFLOW_AWS_ACCESS_KEY_ID", "MINIO_ROOT_USER")
        self._aws_secret_access_key = os.getenv("MLFLOW_AWS_SECRET_ACCESS_KEY", "MINIO_ROOT_PASSWORD")
        self._bucket_name = os.getenv("MLFLOW_AWS_BUCKET_NAME", "mlflow")
