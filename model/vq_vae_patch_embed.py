...
from model.vector_quantizer import ResidualVQLightning, DVAE_Discretizer
...
class VQVAEPatch(Autoencoder):
    ...
    def __init__(
        ...
        hash_model: str = "",
        use_dvae_vq: bool = False,
        dvae_temperature: float = 1.0,
    ):
        if use_dvae_vq:
        ...
        super().__init__(
            ...
            use_dvae_vq=use_dvae_vq,
            dvae_temperature=dvae_temperature,
        )
        ...
        if use_dvae_vq:
            self.vector_quantization = DVAE_Discretizer(
                temperature=dvae_temperature,
            )
        else:
